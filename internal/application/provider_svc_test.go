...
func TestFetchModels_OpenAIFormat(t *testing.T) {
	...
	id, err := uc.Add("TestOpenAI", srv.URL, "", "api-key", "test-token")
	if err != nil {
...
func TestFetchModels_AnthropicFallbackFormat(t *testing.T) {
	...
	uc := setupProviderTest(t)
	id, err := uc.Add("TestAnthropic", srv.URL, "", "api-key", "test-token")
	if err != nil {
		t.Fatalf("Add provider failed: %v", err)
	}
...
func TestFetchModels_Unauthorized(t *testing.T) {
	...
	id, err := uc.providerRepo.Add("TestUnauthorized", srv.URL, "", "api-key", "test-token")
	if err != nil {
		t.Fatalf("AddProvider failed: %v", err)
	}

	err = uc.FetchModels(id, srv.URL, "", "test-token")
	if err == nil {
...
func TestFetchModels_RateLimited(t *testing.T) {
	...
	uc := setupProviderTest(t)
	id, _ := uc.providerRepo.Add("TestRateLimit", srv.URL, "", "api-key", "test-token")
...
func TestFetchModels_ServerError(t *testing.T) {
	...
	uc := setupProviderTest(t)
	id, _ := uc.providerRepo.Add("TestServerError", srv.URL, "", "api-key", "test-token")
...
func TestFetchModels_UnparseableResponse(t *testing.T) {
	...
	uc := setupProviderTest(t)
	id, _ := uc.providerRepo.Add("TestUnparseable", srv.URL, "", "api-key", "test-token")
...
func TestRetryFetch_Success(t *testing.T) {
	...
	uc := setupProviderTest(t)
	id, err := uc.Add("TestRetry", srv.URL, "", "api-key", "test-token")
	if err != nil {
		t.Fatalf("Add provider failed: %v", err)
	}
...
func TestRetryFetch_Failure(t *testing.T) {
	uc := setupProviderTest(t)
	id, _ := uc.providerRepo.Add("RetryFail", "http://localhost:19999", "", "api-key", "bad-token")
	uc.providerRepo.UpdateStatus(id, "error")
...
func TestFetchModels_NoResponseBody(t *testing.T) {
	...
	uc := setupProviderTest(t)
	id, _ := uc.providerRepo.Add("EmptyBody", srv.URL, "", "api-key", "token")

	err := uc.FetchModels(id, srv.URL, "", "token")
	if err == nil {
...
func TestProviderRepo_AddDuplicate(t *testing.T) {
	...
	_, err := providerRepo.Add("Duplicate", "https://api.test.com", "", "key1", "token1")
	if err != nil {
		t.Fatalf("first Add failed: %v", err)
	}

	_, err = providerRepo.Add("Duplicate", "https://api.test.com", "", "key2", "token2")
	if err == nil {

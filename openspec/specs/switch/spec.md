...
## Registry-Based Mutator Resolution

The mutation phase is no longer hardcoded to a JSON `env` block. Instead, `SwitchUseCases.Apply()` resolves the mutator by name from `target_clis.mutator` and delegates to the registered `ConfigMutator` implementation.
